...
/**
 * Ingests real Instagram professional accounts into the influencer database. Sibling to
...
export function InstagramIngest({ disabled }: { disabled?: boolean }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/internal/connectors/instagram/ingest", {
        method: "POST",
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Ingest real accounts — Instagram</CardTitle>
      ...
      <CardContent className="space-y-4">
        <form onSubmit={submit} className="space-y-3">
          <Field
            label="Accounts"
            hint="One per line — @handle, bare handle, or instagram.com URL — Business or Creator accounts only. Up to 10 at a time."
          >
            <Textarea
              value={accounts}
        ...
        {disabled && (
          <p className="text-base text-ink-muted">
            <code className="font-num text-sm">META_IG_TOKEN</code> and{" "}
            <code className="font-num text-sm">META_IG_USER_ID</code> are not both set.
          </p>
        )}
        ...
        {state.status === "done" && (
          <div className="space-y-3">
            <Notice tone="info" title={`${state.ingested} of ${state.results.length} accounts ingested`}>
              ...
              reach any of those on Instagram. Their health scores are computed from the
